...
class VideoInferenceService:
    def __init__(self, websocket_mode=True):
        ...
        self.processing_timeout = 60.0  # Increased to 60 seconds
        # Simplified safety - less aggressive
    ...
    def initialize_from_config(self, config):
        ...
        # Check if we're switching models - if so, force extra cleanup
        current_engine = getattr(self, "last_engine_path", None)
        if current_engine and current_engine != self.config["engine_path"]:
            print(
                f"Detected model switch from {current_engine} to {self.config['engine_path']}"
            )
            print("Performing deep cleanup for model switching...")
        ...
        # Remember the current engine path
        self.last_engine_path = self.config["engine_path"]

        return True

    def _initialize_engine_in_thread(self):
        """Initialize the TensorRT engine in the processing thread with model switching support"""
        try:
            print("Initializing TensorRT engine in processing thread...")

            # Force CUDA cleanup before new model initialization (for model switching)
            try:
                import gc

                gc.collect()
                # Small delay to let any residual operations complete
                time.sleep(0.5)
            except Exception as e:
                print(f"Pre-initialization cleanup warning: {e}")
    ...
    def start_processing(self):
        ...
        # Start watchdog thread for safety
    ...
    def stop_processing(self):
        ...
        if self.processing_thread and self.processing_thread.is_alive():
            ...
            if self.processing_thread.is_alive():
                print(
                    "WARNING: Processing thread did not terminate gracefully - this should NOT happen with new safety measures!"
                )
    ...
    def _add_csv_data(self, measurement_data, timestamp):
        """Add measurement data to CSV collection"""
        csv_row = {
            "tray_number": f"Tray {measurement_data['tray_number']}",
            "timestamp": f"{timestamp:.1f}s",
            "number_of_defected": measurement_data["number_of_defected"],
            "number_of_good": measurement_data["number_of_good"],
            "number_of_missing": measurement_data["number_of_missing"],
    ...
    def _save_csv_data(self):
        """Save CSV data to file with proper error handling"""
        ...
        try:
            ...
            # Make sure filepath is a proper string
    ...
    def emergency_shutdown(self):
        """Simplified emergency stop - less aggressive"""
        ...
        # Try to save CSV before shutdown
        try:
            self._save_csv_data()
        except:
            pass
    ...
    def _watchdog_monitor(self):
        """Smart watchdog - distinguishes between initialization and processing hangs"""
        print("Watchdog monitor started (smart mode)")

        initialization_timeout = 300.0  # 5 minutes for model initialization
        processing_timeout = 60.0  # 1 minute for normal processing
        ...
        while self.is_running and not self.emergency_stop_event.is_set():
            try:
                ...
                # Check if we're still initializing (seg is None)
                if self.seg is None:
                    # We're in initialization phase - use longer timeout
                    if time_since_activity > initialization_timeout:
                        print(
                            f"MODEL INITIALIZATION TIMEOUT after {time_since_activity:.1f}s - Emergency shutdown"
                        )
                        self.emergency_shutdown()
                        break
                else:
                    # We're in normal processing - use shorter timeout
                    if time_since_activity > processing_timeout:
                        print(
                            f"PROCESSING FREEZE DETECTED after {time_since_activity:.1f}s - Emergency shutdown"
                        )
                        self.emergency_shutdown()
                        break
    ...
    def _process_video(self):
        ...
        try:
            ...
            # Proper TensorRT and CUDA cleanup
            if self.seg:
                try:
                    print("Cleaning up TensorRT engine and CUDA context...")

                    # First, clean up TensorRT objects properly
                    ...
                    # Then handle CUDA context cleanup
                    ...
                    # Always set to None after cleanup attempt
                    self.seg = None
    ...
    def _process_inference_frame(self, frame, inference_frame_count, current_timestamp):
        try:
            ...
            # Normal inference - no aggressive timeout checks
            ...
            # Add to CSV data if publishing
            if should_publish:
                self._add_csv_data(measurement_data, current_timestamp)
