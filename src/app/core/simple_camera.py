...
def main():
    parser = argparse.ArgumentParser(description="Donut Measurement Service")
    parser.add_argument("--engine", required=True)
    parser.add_argument("--video", required=True)
    parser.add_argument("--config", required=True)
    parser.add_argument("--button", default="[outer_diameter,inner_diameter]")
    parser.add_argument("--score", type=float, default=0.5)
    parser.add_argument("--mask-thr", type=float, default=0.4)
    parser.add_argument("--alpha", type=float, default=0.45)
    parser.add_argument("--target-fps", type=float, default=30.0)
    parser.add_argument("--csv-output-dir", default="output")
    parser.add_argument("--gap-delay", type=float, default=1.0)
    parser.add_argument("--min-index-count", type=int, default=1)
    parser.add_argument("--match-tol-y", type=float, default=50.0)
    parser.add_argument("--x-forward-slack", type=float, default=6.0)
    parser.add_argument("--left-exit-slack", type=float, default=2.0)
    ...
    config = {
        ...
        "gap_delay": args.gap_delay,
        "min_index_count": args.min_index_count,
        "match_tol_y": args.match_tol_y,
        "x_forward_slack": args.x_forward_slack,
        "left_exit_slack": args.left_exit_slack,
        "gpu_roi": args.gpu_roi,
        "engine_binary_masks": args.engine_binary_masks,
        "workers": args.workers,
    }

    service = VideoInferenceService(websocket_mode=False)
    
    if not service.initialize_from_config(config):
        print("Failed to initialize service")
        return 1

    if not service.start_processing():
    ...
    try:
        while service.is_running:
            ...
            if status.get("is_stuck", False):
                print("Service appears to be stuck, stopping...")
                break
